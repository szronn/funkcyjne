...
decreseTimers cpu = cpu {sound = nsound, delay = ndelay}
    where
        intdelay = fromIntegral (delay cpu) :: Int 
        ndelay = fromIntegral (max 0 (intdelay - 1))
        intsound = fromIntegral (sound cpu) :: Int
        nsound = fromIntegral (max 0 (intsound - 1))
...
execOpcode cpu opcode =
    case opcode of
        ...
        (0xF, x, 0x3, 0x3) ->
            let vx = getReg cpu x in
            incPC $ storeDecimal cpu vx
        -- store registers up to x in memory
        (0xF, x, 0x5, 0x5) ->
            incPC $ storeRegs cpu x
        -- load registers from memory
        (0xF, x, 0x6, 0x5) ->
            incPC $ loadRegs cpu x
        instr -> 
            let idx = (pc cpu) in
...
drawSprite cpu x y n = 
        cpu {vram = screen V.// toWrite, v = (v cpu) V.// [(0xF, carry)]}
    where 
        screen = (vram cpu)
        sprite = getSprite cpu x y n
        memreg = getMemRegion cpu x y n
        carry = if any id $ zipWith (\(_, a) b -> a && b) memreg sprite then 1 else 0
...
checkInput cpu x =
    ...
    where 
        ...
        pressed = List.find snd (zip [0..] keys)
